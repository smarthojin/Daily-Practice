...
# Getting comfortable with these tools early will be very important in becoming an efficient developer. 
# After learning them, these tools will make just about any operation you can think of faster than using a GUI and mouse.

# Iteration Exercises
# We're going to implement some Array methods. There are descriptions for each method, but it may 
# help to also consult the official Ruby docs for Enumerable and Array. These are good resources in general, 
# and useful reading during this first week.
...
# Start by opening the terminal and changing your directory to the Desktop using the cd command. Create a new 
# directory for your work using the mkdir command and then open a new file in VS Code called enumerables.rb using the code command.

# Enumerable
# My Each
# Extend the Array class to include a method named my_each that takes a block, calls the block on every element of
#  the array, and returns the original array. Do not use Enumerable's each method. I want to be able to write:
...
# My Join
# my_join returns a single string containing all the elements of the array, separated by the given string separator. If
#  no separator is given, an empty string is used.
