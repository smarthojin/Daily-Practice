...
def filter_lengths(str, length=5)
   str.select{|char| char.length >= length}
   # select method is anotherloop that takes in a condtional for booleans and 
   # select and returns ones that equal to the conditioinal
end
